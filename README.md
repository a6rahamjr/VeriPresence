...
unknown-person detection. It provides a tested ML pipeline, versioned artifacts, SQLite
events, FastAPI inference, and camera/video operation.
...
### Option A: Import filename-labeled images

For an existing flat image directory:

```bash
veripresence-import ../face-images \
  --destination data/raw/enrollment \
  --minimum-images 2
```

The importer converts names such as `Person001.jpg` into
`data/raw/enrollment/person/person_001.jpg`. Groups below the minimum image count are
excluded.
...
The synthetic data contains seeded class, grayscale/color, and capture variation. It
validates the software workflow; it is not a face-recognition benchmark.
...
No browser or keyboard automation is used.
...
- Synthetic enrollment data completed training and artifact export.
- Known synthetic samples were accepted by inference.
- Unusable dark and blurred inputs were rejected by the quality gate.

These checks demonstrate the software workflow. They are not a production certification
or a substitute for a larger session-separated evaluation.
