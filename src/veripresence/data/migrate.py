...
def migrate_legacy_images(
    ...
    if not source.exists():
        raise FileNotFoundError(f"Legacy image directory not found: {source}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Import filename-labeled legacy face images.")
    parser.add_argument("source", type=Path, help="Directory containing legacy images.")
    parser.add_argument(
        "--destination",
        type=Path,
    ...
    report = migrate_legacy_images(args.source, args.destination, args.minimum_images)
